...
@app.route('/mobskills', methods=["GET"],)
@cross_origin()
def getmobskills():
    # set limit
    limit = 100
    ...
    query = "SELECT n.id, n.level, n.skill_id, s.name, s.icon FROM npcskills as n INNER JOIN skillnam as s on (n.SKILL_ID=s.SKILL_ID AND n.LEVEL=s.LEVEL)" 
    if npc_id != None:
        query += f" WHERE n.NPC_ID = '{npc_id}'"
    if page!=None:
        query += f" LIMIT {limit} OFFSET {offset}"
    
    total_rows_query = "SELECT COUNT (n.id) FROM npcskills as n INNER JOIN skillnam as s on (n.SKILL_ID=s.SKILL_ID AND n.LEVEL=s.LEVEL)"
    if npc_id != None:
        total_rows_query += f" WHERE n.npc_id = {npc_id}"
