...
def getmobdrop():
    ...
    query = "SELECT dr.*,i.icon,i.name FROM droplist as dr LEFT JOIN items as i ON i.item_id = dr.item_id " 
    if npc_id != None:
        query += f" WHERE dr.npc_id = '{npc_id}'"
    if page!=None:
        query += f" LIMIT {limit} OFFSET {offset}"
    
    total_rows_query = "SELECT COUNT (dr.id) FROM droplist as dr LEFT JOIN items as i ON i.item_id = dr.item_id "
    if npc_id != None:
        total_rows_query += f" WHERE dr.npc_id = '{npc_id}'"
